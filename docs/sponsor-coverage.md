...
| Bonus Quest Champion | Conditional target | Off-Brand is strong; no-cloud-API design, public build trace, field-note report, and optional `llama.cpp` runway can stack merit badges only if the final rules still expose those badges and the public evidence is shown. | Do not chase fine-tuning, tiny-model, or runtime badges with fake data or weaker output. |
