...
| Llama Champion | Do not claim yet | Optional `USE_LLAMA_CPP=1` backend now supports direct `llama-cli` via `LLAMA_CPP_BACKEND=cli` and `openbmb/MiniCPM4.1-8B-GGUF:Q4_K_M`; `python3 scripts/llama_runtime_check.py` now passes with installed Homebrew `llama.cpp`. | Needs MiniCPM GGUF non-fallback generation through llama.cpp before claiming. |
