...
| --- | --- | --- | --- |
| Backyard AI | Primary target | The product solves a concrete low-time study panic workflow for students, with a one-input/one-packet path, visible proof target, live coach, and study receipt. This is stronger than Thousand Token Wood because usefulness is the core, not a playful AI toy. | Keep the demo focused on student usefulness, not evidence collection. |
...
| OpenBMB | Claim live ZeroGPU generation now | Default model path is `openbmb/MiniCPM-V-4_5`, under the `<=32B` rule and aligned with OpenBMB/MiniCPM special-prize judging. The app also keeps an optional official OpenBMB MiniCPM4 0.5B GGUF path through llama.cpp. | Preserve the CPU fallback and avoid spending the 40 min/day quota on unnecessary repeated tests. |
| NVIDIA Nemotron Quest | Claim selectable fallback now | The official award is for standout Nemotron builds. An optional `nvidia/Nemotron-Mini-4B-Instruct` engine is selectable in the UI and verified live on ZeroGPU (returns real model-written plan and drills on CUDA). | None. |
| Cohere | Do not target unless criteria appear | Cohere is currently tracked as a supporting sponsor/cash contributor, not a confirmed model-use bounty. Optional `USE_COHERE_REVIEW=1` quality-review hook exists and stays disabled by default; response parsing is tested against the official `message.content[].text` v2 shape. | No product work needed now. Do spend demo surface on Cohere or claim live integration unless official criteria appear and a key-backed smoke passes. |
...
| Off-Brand | Claim now | Custom Gradio CSS/HTML, clearly labeled sample scenarios, panic-pattern readout, study receipt, a final-sheet card with first-action/proof-target/guardrail sections, a runtime badge that names the exact model and its size, a hackathon footer strip, and collapsed claim proof rather than a default Gradio layout. | Repeat final visual QA before final submission. |
| Tiny Titan | Claim selectable sub-4B now | A sub-4B path is selectable in the UI (`openbmb/MiniCPM5-1B`), and verified live on ZeroGPU (returns real model-written plan and drills, and prints the 1B parameter size as proof). | None. |
