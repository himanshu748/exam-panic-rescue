...
| Off the Grid | Claim only with cloud hooks disabled | Local processing by default; no external API key required for the demo. Optional model/runtime hooks are explicit opt-ins, and CPU-only HF Spaces use the deterministic fallback by default. | Verify final Space path does not require external calls. |
