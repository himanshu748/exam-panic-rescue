...
| Tiny Titan | Available, claim only if demoed | A sub-4B path now ships: set `MODEL_ID=openbmb/MiniCPM4-0.5B` (0.5B) or `openbmb/MiniCPM5-1B` (1B), and the runtime note prints the model size as proof. Default stays `openbmb/MiniCPM4.1-8B` for answer quality. | Record a short ≤4B smoke and show it in the demo before claiming. |
